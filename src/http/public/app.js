...
async function deepAnalyze() {
  ...
  try {
    ...
    state.decisionId = (typeof state.decisionId === "string" && state.decisionId.startsWith("decision_")) ? state.decisionId : `decision_${started}_${Math.random().toString(36).slice(2, 8)}`; const snapshot = { analysisId: state.decisionId, decisionId: state.decisionId, requestId: frameId, sessionId: state.liveSessionId, candleId: state.lastCandleId, candleSeconds: CANDLE_SECONDS, visibleWindowSeconds: VISIBLE_WINDOW_SECONDS, horizonSeconds: EXPIRATION_SECONDS, predictionHorizonSeconds: EXPIRATION_SECONDS, timestampMs: Date.now(), symbol: state.lastContext?.symbol || "UNAVAILABLE", chartFrame: { frameId }, quantitativeFeatures: { availability: "SCREEN_MOTION_ONLY", frames: state.frames.map((item) => ({ ...item.stats, sample: undefined })) } };
