...
function channelManualPosition(manual) {
  ...
  channel.state = "IN_POSITION"; channel.manualEntry = { price: priceRow.value, confidence: priceRow.confidence, source: priceRow.source, timestamp: now, signalId: signal.signalId }; channel.settlementAt = now + OP_HORIZON_MS; channel.metrics.entryConfirmed += 1;
...
async function observe() {
  ...
  try {
    ...
    const fastResponse = await api("/api/fast/decision", { method: "POST", body: JSON.stringify({
      ...
      previousMacroState: state.macroState || null,
    }) });
...
async function trainIfActive(snapshot, analysis, stat) {
    if (!state.training) return; const sizing = suggestedStake(analysis); analysis.profiles = analysis.profiles || null; const causalPrice = latestCausalPrice(snapshot.timestampMs); const referencePrice = causalPrice ? causalPrice.value : null; const referencePriceSource = causalPrice ? causalPrice.source : "UNAVAILABLE"; const referencePriceConfidence = causalPrice ? causalPrice.confidence : 0; if (referencePrice === null) console.warn("ENTRY_PRICE_UNAVAILABLE", JSON.stringify({ candleId: snapshot.candleId, reason: "no_causal_price_observation", decision: analysis.decision })); else if (analysis.decision === "BUY" || analysis.decision === "SELL" || ["BUY", "SELL"].includes(analysis.directionalLean)) console.info("ENTRY_PRICE_LOCKED", JSON.stringify({ price: referencePrice, priceObservationId: causalPrice.frameId, priceFrameId: causalPrice.frameId, priceObservedAt: causalPrice.timestamp, lockTimestamp: Date.now(), ageMs: snapshot.timestampMs - causalPrice.timestamp, confidence: referencePriceConfidence, source: referencePriceSource, candleId: snapshot.candleId })); const result = await api("/api/training/analyze", { method: "POST", body: JSON.stringify({ trainingSessionId: state.training.id, snapshot: { ...snapshot, symbol: state.lastContext?.symbol || "UNAVAILABLE", referencePrice, referencePriceSource, priceConfidence: referencePriceConfidence, framesHash: frameHash(stat), features: { motion: stat } }, analysis, suggestedStake: sizing.amount || null, profiles: analysis.profiles || null, selectedProfile: analysis.selectedProfile || state.profile, regime: analysis.regime || null, rawConfidence: analysis.rawConfidence ?? null, calibratedConfidence: analysis.calibratedConfidence ?? null, synthetic: false, payoutAtDecision: null, breakEvenWinRate: null }) }); state.training = result; renderTraining(result);
}
...
function openAdminDialog() { const dialog = $("adminDialog"); if (!dialog) return; $("adminKeyBox").hidden = true; $("adminKeyValue").textContent = ""; delete $("adminKeyValue").dataset.keyId; $("adminGenerateStep").hidden = true; $("adminLoginStep").hidden = false; $("adminSecretInput").value = ""; $("adminLoginError").textContent = ""; dialog.showModal(); }
async function adminLogin() { const secret = $("adminSecretInput")?.value || ""; if (!secret) return; $("adminLoginError").textContent = ""; try { await api("/api/live/admin/login", { method: "POST", body: JSON.stringify({ adminKey: secret }) }); $("adminSecretInput").value = ""; $("adminLoginStep").hidden = true; $("adminGenerateStep").hidden = false; liveStatus("READY", "Sessão administrativa autorizada."); } catch (error) { $("adminLoginError").textContent = String(error.message || "Falha de autenticação"); } }
function closeAdminDialog() { const dialog = $("adminDialog"); $("adminKeyValue").textContent = ""; delete $("adminKeyValue").dataset.keyId; $("adminKeyBox").hidden = true; $("adminRevokeConfirm").hidden = true; $("adminSecretInput").value = ""; if (dialog?.open) dialog.close(); }
async function generateLiveKey(rotate = false) {
  const dialog = $("adminDialog"); if (!dialog?.open) { openAdminDialog(); liveStatus("LOADING", "Autentique para gerar a chave."); return; }
  if ($("adminGenerateStep").hidden) { liveStatus("LOADING", "Autentique primeiro."); return; }
  liveStatus("LOADING", rotate ? "Rotacionando chave…" : "Gerando chave…");
  try {
    const keyBox = $("adminKeyValue"); const previousId = keyBox.dataset.keyId;
...
async function rotateLiveKey() { const dialog = $("adminDialog"); if (!dialog?.open) { openAdminDialog(); return; } if (!$("adminGenerateStep").hidden) return generateLiveKey(true); }
...
$("shareButton").addEventListener("click", shareScreen); $("stopShareButton").addEventListener("click", stopScreen); $("recropButton").addEventListener("click", openManualCrop); $("startTrainingButton").addEventListener("click", () => startTraining().catch((error) => text("trainingNote", error.message))); $("stopTrainingButton").addEventListener("click", stopTraining); $("liveGenerateKey")?.addEventListener("click", () => openAdminDialog()); $("liveRotateKey")?.addEventListener("click", rotateLiveKey); $("liveRevokeKey")?.addEventListener("click", revokeLiveKey); $("liveCopyInstructions")?.addEventListener("click", copyLiveInstructions); $("liveCopyPrompt")?.addEventListener("click", copyOpenCodePrompt);
$("adminLoginButton")?.addEventListener("click", adminLogin); $("adminCancelLogin")?.addEventListener("click", closeAdminDialog); $("adminGenerateConfirm")?.addEventListener("click", () => generateLiveKey(false)); $("adminRotateConfirm")?.addEventListener("click", () => generateLiveKey(true)); $("adminRevokeConfirm")?.addEventListener("click", revokeLiveKey); $("adminClose")?.addEventListener("click", closeAdminDialog); $("adminCopyKey")?.addEventListener("click", async () => { const value = $("adminKeyValue")?.textContent || ""; if (!value) return; try { await navigator.clipboard.writeText(value); liveStatus("READY", "Chave copiada. Guarde-a agora."); } catch { liveStatus("ERROR", "Não foi possível copiar."); } });
refreshAdminSession(); bindManualCrop(); $("profileSelect")?.addEventListener("change", (event) => applyProfile(event.target.value)); applyProfile(state.profile); checkBackend(); updatePipeline(); restoreTraining(); refreshLiveStatus(); setInterval(refreshLiveStatus, 30000); setInterval(refreshTraining, 5000); window.addEventListener("beforeunload", stopScreen);
