...
async function generateLiveKey(rotate = false) { const adminKey = window.prompt("Chave administrativa da interface (não será armazenada):"); if (!adminKey) return; liveStatus("LOADING", rotate ? "Rotacionando chave…" : "Gerando chave…"); try { const output = $("liveKeyOutput"); const previousId = output.dataset.keyId; const preset = $("livePreset")?.value || "OPENCODE_FULL_DIAGNOSTIC"; const days = Number($("liveExpiration")?.value) || 0; const payload = { name: "OpenCode Live Access", preset }; if (days > 0) payload.expiresAt = new Date(Date.now() + days * 86_400_000).toISOString(); const result = await api("/api/live/admin/keys", { method: "POST", headers: { "x-live-admin-key": adminKey }, body: JSON.stringify(payload) }); output.hidden = false; output.textContent = result.key; output.dataset.keyId = result.id; output.dataset.adminKey = adminKey; $("liveRevokeKey").hidden = false; if (rotate && previousId) { try { await api(`/api/live/admin/keys/${encodeURIComponent(previousId)}/revoke`, { method: "POST", headers: { "x-live-admin-key": adminKey }, body: "{}" }); } catch { /* previous key may already be gone */ } } liveStatus("READY", "Chave exibida uma única vez. Copie-a com segurança."); } catch (error) { liveStatus("ERROR", error.message); } }
...
$("shareButton").addEventListener("click", shareScreen); $("stopShareButton").addEventListener("click", stopScreen); $("recropButton").addEventListener("click", openManualCrop); $("startTrainingButton").addEventListener("click", () => startTraining().catch((error) => text("trainingNote", error.message))); $("stopTrainingButton").addEventListener("click", stopTraining); $("liveGenerateKey")?.addEventListener("click", () => generateLiveKey(false)); $("liveRotateKey")?.addEventListener("click", rotateLiveKey); $("liveRevokeKey")?.addEventListener("click", revokeLiveKey); $("liveCopyInstructions")?.addEventListener("click", copyLiveInstructions); $("liveCopyPrompt")?.addEventListener("click", copyOpenCodePrompt); bindManualCrop(); $("profileSelect")?.addEventListener("change", (event) => applyProfile(event.target.value)); applyProfile(state.profile); checkBackend(); updatePipeline(); restoreTraining(); refreshLiveStatus(); setInterval(refreshLiveStatus, 30000); setInterval(refreshTraining, 5000); window.addEventListener("beforeunload", stopScreen);
