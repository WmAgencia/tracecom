...
### ETAPA 3 - BLOQUEADOR 2 RESOLVIDO COM EVIDENCIA (esta sessao) - retomada EXATA
- PROVA DO FEED (respostas A-D): A) SIM - IQ envia candle 5s NATIVO: relay/iqoption-ws.mjs:31 export const CANDLE_SIZE_SECONDS = 5; o runtime assina subscribeCandles(activeId, size=CANDLE_SIZE_SECONDS) (subscribeMessage candle-generated com routingFilters) e filtra Number(event.msg?.size) === CANDLE_SIZE_SECONDS (:810) -> #onCandleEvent (:799) -> #ingestCandle (:819, sizeSeconds=CANDLE_SIZE_SECONDS no normalizeCandle) -> #pipeClosedCandle -> RuntimeIntelligence.onClosedCandle. B) NAO ha stream de tick/quote no cliente WS (sem subscribeQuotes/quote; teste estatico prova ausencia) -> NENHUM agregador 5s e necessario. C) NAO e 1m-only. D) streams: candle-generated 5s por ativo + commission/balance (nao-preco).
- EVIDENCIA NUMERICA (scripts/stage3-feed-granularity-tests.mjs 11/11): raw_source=candle-generated(5s nativo), observed_intervalMs=5000, candles=2160, coverageMs=10795000 (3h menos 1 intervalo, dentro da tolerancia), hydration=HYDRATION_READY, health state=READY + intelligenceReady=true + observedIntervalMs=5000; allowsExecution -> DENY STRATEGY_NOT_ACTIVE (V2 PENDING_IMPLEMENTATION). Nenhuma ordem.
- CONSEQUENCIA: V2 NAO esta BLOCKED_BY_5S_DATA_SOURCE (o feed e 5s nativo) -> caminho livre para: (1) hydration loader real (fonte candidata primaria: candles 5s ja normalizados em ctx.candles durante a sessao + candles-archive para persistencia; validar se o archive guarda 5s ou 1m antes de usar); (2) ligar decisions() -> SinglePath -> requestOrder no loop (dedup/lock por ativo + race killSwitch); (3) paridade full-runtime PRACTICE/REAL + causalidade runtime + fixtures restantes; (4) agentExpirySeconds -> read-only (autoridade OPERATIONAL_EXPIRY_SECONDS/Binary300Timing); rename submitLabPracticeOrder; (5) strategyHash V2 (incluir readiness/gap policy) + manifesto + READY_FOR_DEPLOY (NUNCA ACTIVE); (6) Etapa 4 frontend; (7) Etapa 5 DB/invariants/CI/docs/E2E PRACTICE PATH_TEST/Binary300 broker validation; (8) Etapa 6 deploy/hydration real/REAL DISARMED; ACTIVE somente se 5s validado em producao + CI green + REAL disarmed; depois FREEZE.
- INVARIANTES: blitz 0, 300s unico, capability operacional de broker = 0 (fail-closed), REAL nunca tocado, V2 PENDING_IMPLEMENTATION executable=false, baseline intacta, sem confidence, nenhuma ordem enviada.
- COMMIT DE RETOMADA: este (git log -1 apos push).

### ETAPA 3 FECHADA + ETAPA 4 + ETAPA 5 (2026-09-22/23) - retomada EXATA
- PONTO DE PARTIDA: 26a96d8 (working tree clean, remote sincronizado). Commits desta sessao: 55354e4 (caminho unico + persistencia 5s), e8d9b9b (versoes de semantica), 89c44d0 (strategyHash + manifesto READY_FOR_DEPLOY), + este commit (frontend/DB/CI/docs).
- ETAPA 3 FECHADA (falta apenas operar de verdade na Etapa 6):
  1. `decisions() -> SinglePath -> requestOrder` LIGADO: `relay/execution/intelligence-dispatch.mjs` (dedup por snapshotId via claim+pipeline.markSubmitted; lock por ativo — EURUSD em voo nao bloqueia USDJPY; revalidacao do MESMO gate imediatamente antes do submit -> kill-switch race DENY com requestOrder calls=0; WAIT nunca entra; PRACTICE/REAL com mesmo fingerprint/expiracao; REAL exige realArmed+contexto REAL). Runtime: `pumpIntelligenceDecisions()` chamado no candle fechado (#pipeClosedCandle) e no tick 1s.
  2. HYDRATION CROSS-RESTART REAL: migration `051_operational_candles_5s.sql` + `relay/intelligence/candle-store.mjs` (unique asset+interval+at; retencao 3h30; flush 2s; prune no runDbMaintenance). RuntimeIntelligence.start usa loader do CandleStore; `#ensureIntelligenceHydration()` no boot/setMarket/reopen (so OTC habilitado; NORMAL nao opera). asset-pipeline recupera READY por tempo ao vivo (nunca promove INTERVAL_INCOMPATIBLE; FUTURE_CANDLES e hard fail).
  3. `agentExpirySeconds` REMOVIDO (autoridade unica OPERATIONAL_EXPIRY_SECONDS=300). `submitLabPracticeOrder` -> `submitPathTestOrder` (PRACTICE-only, 300s, testOnly/excludedFromStats, REAL -> DRY_RUN). Allowlist unica `relay/execution/operational-policy.mjs` (OPERATIONAL_V2_PLUS_TEST_PATHS: intelligence + pathtest + ui:smoke + agent-v2 path-test).
  4. computeExpiration(sec,5) agora e ESTRITO no bucket de 5 min (optionTypeId 3/turbo 5m, reference tracecom/binary300-bucket); requestOrder default = OPERATIONAL_EXPIRY_SECONDS.
  5. strategyHash REAL da V2: `sha256:3e9364e2d6e7b1e38ea3900a3a1c7a7e3be778978c8d4d0e563cbcb9645daeb0` (script `scripts/stage3-strategy-hash.mjs`; cobre 7 arquivos de decisao + policy 300s/5s/3h/readiness/gap + baseline congelada; sem frontend/CSS/deploy/logs). Manifesto `estrategias/strategy-versions/PULLBACK_4060_300_AGENTIC_V2.json` = READY_FOR_DEPLOY, executable=false, stats zerados, sourceCommit e8d9b9b.
  6. TESTES NOVOS: dispatch 28, hydration 16, runtime-integrity 19 (causalidade com dataset real 1m truncado; startup failures; wiring), broker-contract 8, strategy-hash 12, frontend 23, e2e-practice 10, invariants 22. Runner unico: `node scripts/run-all-tests.mjs` (19 alvos, todos verdes) + `npm run build`.
- ETAPA 4 (frontend): `src/http/public/grid.html` reescrito — nav MESAS/LOG/HISTORICO/CONFIGURACOES; grid renderiza productState do backend (`/api/iq/intelligence/assets`); LOG com Asset Agent + 5 especialistas + Consensus + Execucao (WAIT aparece aqui); HISTORICO lista Hora/Ativo/Direcao/Resultado/PnL/Conta/Strategy Version + detalhe (AssetContext/Specialists/Consensus/Snapshot/Execution/Settlement); conta PRACTICE/REAL so muda selectedAccount (ARMAR REAL separado com frase); ZERO Blitz/duracao/confidence/seletor antigo/LAB.
- ETAPA 5 (DB/CI/docs): migration `052_operational_trade_tracking.sql` (strategy_version/hash/stats_epoch/snapshot_hash/decision_snapshot/test_only/excluded_from_stats em iq_executions); `requestOrder` persiste o snapshot operacional (nunca os candles); `/api/iq/strategy/stats` separa V2 (exclui PATH_TEST); `/api/iq/intelligence/assets` view operacional; CI `.github/workflows/ci.yml` (build + node --check + run-all-tests + baseline imutavel); docs/cofre atualizados; agent-policy com red zones novas (intelligence/execution/migrations/grid).
- INVARIANTES PRESERVADOS: blitz 0, 300s unico, REAL fail-closed/nunca enviado, V2 READY_FOR_DEPLOY (executable=false, NAO ACTIVE), baseline intacta, sem confidence, inteligencia sem conta/broker, nenhuma ordem real.
- FALTA (Etapa 6, ordem EXATA): (1) push/deploy pela infra existente (Dockerfile -> relay/server.mjs; Vercel serve dist/http/public); (2) prod health: /health, DB, WS IQ, native 5s (observedIntervalMs=5000), Binary OTC discovery, hydration (assets READY), AssetPipeline/FeatureEngine/Consensus, DecisionSnapshot, SinglePath, AccountRouter, grid/log/historico; (3) validacao Binary300 PRACTICE contra a IQ (serverTime/targetExpiry/bucket/deadline/safe cutoff; se rejeitar, corrigir COM EVIDENCIA; NUNCA fallback 60s; NUNCA REAL); (4) E2E PRACTICE real (PATH_TEST testOnly/excludedFromStats); (5) REAL pos-deploy = realArmed=false; (6) SO ENTAO READY_FOR_DEPLOY -> ACTIVE no manifesto (novo deploy/restart para o runtime ler) + FREEZE da V2 (mudanca futura = V3 + novo hash + novo statsEpoch). Se qualquer gate falhar: NAO ACTIVE (fail-closed).
- COMMIT DE RETOMADA: este (git log -1 apos push).

### ETAPA 6 (deploy) + BUGFIX DE PROD (2026-09-23) - retomada EXATA
- DEPLOY EXECUTADO pela infra existente: `node scripts/deploy-prod.mjs --skip-tests` -> Railway (`railway up -d -s tracecom-live-relay`, deployment SUCCESS) + Vercel (`vercel --prod`). CI do GitHub Actions ficou VERDE em abab61a (build + node --check + run-all-tests + baseline).
- BUG DE PROD ENCONTRADO E CORRIGIDO (evidencia em `railway logs`): `IQ_WS_HANDLER_ERROR candle-generated Cannot read properties of undefined (reading 'length')` em TODO candle. Causa raiz: modulos shadow desligados (timingShadow/scenarioShadow nulos) com optional chaining incompleto -> `#observeTimingShadow` (actives.length), `#observeScenarioTimingIntersectionsForMarket` (`for ... of this.timingShadow?.list()`), `timingPolicyStatus` (`.map`/`.persist`) e `scenarioShadowStatus` (`.list()`). Corrigido com `?? []`/`?.persist ?? null`. Regressao coberta em `stage5-e2e-practice-tests.mjs` (6 candles sem excecao + featureState fresh). Repro local: `ingestEvent("candle-generated", ...)` 6x.
- research-smoke atualizado para a realidade reconstruida (removidos 404 legados agents-v4/rsi-agents-v4/v2-live/blitz; adicionados /api/iq/intelligence, /api/iq/intelligence/assets, /api/iq/strategy/stats, /api/iq/execution-routing, /api/iq/status).
- V2 NAO FOI ATIVADA (fail-closed, correto): os gates de ativacao incluem "Binary300 PRACTICE accepted" e "assets READY em prod", que exigem uma ordem PRACTICE real (PATH_TEST) via API admin e inspecao do runtime em prod. Nesta sessao NAO ha acesso ao admin do relay (TOKEN_SIGNING_SECRET) nem sessao de broker; nao se fabrica trade nem se ativa estrategia sem essa validacao. V2 permanece `READY_FOR_DEPLOY` (executable=false) e REAL desarmado.
- PROXIMO PASSO EXATO (Etapa 6 restante, exige operador/admin): (1) apos o deploy do bugfix, checar `railway logs` -> sem IQ_WS_HANDLER_ERROR; procurar `ASSET_INTELLIGENCE_HYDRATE` com ready>0 e `observedIntervalMs=5000`; (2) com admin: `GET /api/iq/intelligence/assets` -> productStates reais e assets READY; `POST /api/iq/test-order` (PRACTICE-only 300s, testOnly/excludedFromStats) OU `agenticTestRun` PATH_TEST -> validar ACK da IQ no contrato do bucket de 5 min; se a IQ rejeitar, corrigir Binary300Timing/computeExpiration COM EVIDENCIA (nunca fallback 60s, nunca REAL); (3) confirmar REAL desarmado (`realArmed=false`) e selectedAccount; (4) SO ENTAO editar o manifesto para `"status": "ACTIVE"` (o runtime le o manifesto no boot) + novo deploy/restart + FREEZE da V2; (5) qualquer mudanca futura = V3 + novo strategyHash + novo statsEpoch.
- INVARIANTES: blitz 0, 300s unico, REAL fail-closed/nunca enviado, V2 READY_FOR_DEPLOY (nao ACTIVE), baseline intacta, sem confidence, inteligencia sem conta/broker.
- COMMIT DE RETOMADA: este (git log -1 apos push).
