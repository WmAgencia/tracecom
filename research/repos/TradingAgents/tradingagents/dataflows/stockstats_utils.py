...
from .akshare_data import _parse_a_share_symbol
...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    For A-share stocks (.SS, .SZ), uses akshare; otherwise uses yfinance.
    """
    ...
    # Check if this is an A-share stock
    is_a_share = canonical.endswith('.SS') or canonical.endswith('.SZ') or canonical.endswith('.SH')

    if is_a_share:
        # Use akshare for A-share stocks
        return _load_ohlcv_akshare(symbol, curr_date, config)
    else:
        # Use yfinance for other markets
        return _load_ohlcv_yfinance(symbol, curr_date, canonical, safe_symbol, config)


def _load_ohlcv_yfinance(symbol, curr_date, canonical, safe_symbol, config):
...
def _load_ohlcv_akshare(symbol, curr_date, config):
    """Load OHLCV using akshare for A-share stocks."""
    ...
    if data is None:
        try:
            ...
            if df.empty:
                raise NoMarketDataError(
                    symbol, code, "akshare returned no rows"
                )
            ...
            raise NoMarketDataError(
                symbol, code, f"akshare error: {str(e)}"
            )
