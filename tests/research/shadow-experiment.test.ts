...
describe("shadow experiment engine (causal, accounting-safe)", () => {
  ...
  it("keeps the probability distribution normalized and WAIT first-class", () => {
    ...
    expect(rows.map((r) => r.strategyVersion)).toContain("shadow-reversion-v5");
    expect(rows.map((r) => r.strategyVersion)).toContain("shadow-reversion-v6");
    expect(rows.map((r) => r.strategyVersion)).toContain("shadow-snapback-v1");
