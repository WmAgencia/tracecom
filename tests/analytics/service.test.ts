...
describe("AnalyticsService + DecisionRepository", () => {
  it("registra uma decisão pendente", async () => {
    ...
    expect(rec.evaluationAttempts).toBe(0);
    expect(rec.providerId).toBeNull();
    store.close();
  });
  ...
  it("valida apenas decisões cujo horizonte já decorreu (dados reais, sem inventar)", async () => {
    ...
    const candles = [
      ...
      mk(T0 + 4 * M, 106), mk(T0 + 5 * M, 108),
    ];
    const svc = new AnalyticsService({ persist: repo, candles: candleSource(candles), cfg: { minMovePct: 0.5, lookback: 100 } });
  ...
  it("decisão recente (horizonte não decorrido) fica pendente", async () => {
    ...
    const svc = new AnalyticsService({ persist: repo, candles: candleSource([mk(future, 100)]), cfg: { minMovePct: 0.5, lookback: 100 } });
