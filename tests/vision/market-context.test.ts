...
describe("MarketContext provenance", () => {
  ...
  it("keeps runtime asset detection dynamic and resets segment buffers on change", () => { const app = readFileSync("src/http/public/app.js", "utf8"); expect(app).toContain("const pair = raw.match"); expect(app).toContain("const pairCanonical"); expect(app).toContain("state.priceObservations = []"); expect(app).toContain("state.priceOutlierCandidates = []"); expect(app).toContain("state.contextSegmentId = `seg_"); });
});
