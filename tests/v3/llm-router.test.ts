...
describe("V3 llm router — free pool", () => {
  it("rotas: 6 especialistas deterministicos; Consensus Groq->NVIDIA(deepseek)->NVIDIA(glm-flash)->Alibaba(ultimo)", () => {
    ...
    expect(FREE_ROLES_CONFIG.CONSENSUS_FINAL[1]).toMatchObject({ provider: "nvidia", model: "deepseek-ai/deepseek-v4.1-flash" });
  ...
  it("choose: Groq primeiro; falha => NVIDIA deepseek; falha => NVIDIA glm-flash; falha => Alibaba; todos em cooldown => null", () => {
    ...
    router.report({ provider: "nvidia", model: "z-ai/glm-5.3-flash", httpStatus: 402, status: "ERROR" });
