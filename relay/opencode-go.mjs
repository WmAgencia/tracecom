...
export function resolveModel(config) {
  ...
  return /^[a-z0-9./\-]{2,96}$/i.test(model) ? model : (config?.provider === "groq" ? GROQ_DEFAULT_MODEL : config?.provider === "zen" ? ZEN_DEFAULT_MODEL : config?.provider === "nvidia" ? (process.env.NVIDIA_MODEL || NVIDIA_DEFAULT_MODEL) : config?.provider === "alibaba" ? (process.env.ALIBABA_MODEL || ALIBABA_DEFAULT_MODEL) : DEFAULT_MODEL);
}
...
export async function runTextProvider(pool, { system = "You are a cautious quantitative analyst. Do not provide execution instructions.", prompt, maxTokens = 1500, requestId = null, sessionContext = {}, temperature = null, responseFormat = null, reasoningEffort = null, timeoutMs = null, provider = null, model = null } = {}) {
  ...
  const apiKey = requestedProvider === "groq"
    ...
    : requestedProvider === "nvidia"
      ? (process.env.NVIDIA_API_KEY || (dbConfig?.provider === "nvidia" ? dbConfig.apiKey : null))
  ...
  const sessionId = sessionFor({ ...sessionContext, requestId });
  if (!apiKey || (config.provider !== "openCodeGo" && config.provider !== "groq" && config.provider !== "zen" && config.provider !== "alibaba" && config.provider !== "nvidia")) return { status: "ERROR", reason: "PROVIDER_NOT_CONFIGURED", requestId, sessionId, model: null, text: null, parsed: null, latencyMs: null, usage: null, finishReason: null };
