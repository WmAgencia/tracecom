...
import { startExperimentLoop, experimentStatus, experimentTrades, retroEvaluate } from './experiment.mjs';
import { frozenTick, getSelection, setSelection, frozenStats, selectionAudit, frozenHistory, maybeAutoSelect } from './frozen-strategies.mjs';
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if(url.pathname === '/api/shadow/experiment/status' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); return reply(res,200, await experimentStatus(pool)); }
    if(url.pathname === '/api/shadow/experiment/trades' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); return reply(res,200, await experimentTrades(pool, Number(url.searchParams.get('limit')) || 100)); }
    if(url.pathname === '/api/shadow/experiment/backfill' && req.method === 'POST') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); const input=await body(req); return reply(res,200, await retroEvaluate(pool, { strategies: Array.isArray(input.strategies) ? input.strategies.map(String) : null, maxCreations: Math.max(1, Math.min(1000, Number(input.maxCreations) || 400)) })); }
    ...
    if(sessionTimeline && req.method === 'GET') {
      ...
      const [events,frames,decisions,settlements,samples,logs,runs]=await Promise.all([
        ...
        pool.query('SELECT decision_id as "decisionId",timestamp as at,direction,confidence,probability_source as "probabilitySource",p_buy,p_sell,p_wait,trace_id as "traceId",market_event_id as "marketEventId",candle_id as "candleId",frame_id as "frameId",decision_source as "decisionSource",deep_requested as "deepRequested",deep_executed as "deepExecuted" FROM live_decisions WHERE session_id=$1 ORDER BY id',[sid]),
        pool.query('SELECT s.decision_id as "decisionId",s.timestamp as at,s.result,s.entry_price,s.settlement_price,g.entry_price_observation_id as "entryPriceObservationId",g.settlement_price_observation_id as "settlementPriceObservationId",g.ground_truth_id as "groundTruthId",g.trace_id as "traceId" FROM live_settlements s LEFT JOIN ground_truths g ON g.signal_id = s.decision_id WHERE s.session_id=$1 ORDER BY s.id',[sid]),
    ...
    if(url.pathname === '/api/live/session') { const started=Date.now(), key=await authenticate(req,'live:session:read'); if(!key && req.headers['x-relay-admin'] !== admin)return reply(res,401,{error:'unauthorized'}); const out=await currentSnapshot(); await accessLog(key.id,url.pathname,200,started,req.headers['x-client-label']); return reply(res,200,out); }
    ...
    if(url.pathname === '/api/strategies/selection' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); return reply(res,200,{ selection: await getSelection(pool), audit: await selectionAudit(pool, 50) }); }
    if(url.pathname === '/api/strategies/selection' && req.method === 'PUT') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); const input = await body(req); const result = await setSelection(pool, { family: input.family, horizonSeconds: Number(input.horizonSeconds), reason: input.reason, actor: input.actor, mode: input.mode }); return reply(res, result.ok ? 200 : 400, result); }
    if(url.pathname === '/api/strategies/stats' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); const [selection, variants, audit] = await Promise.all([getSelection(pool), frozenStats(pool, Number(url.searchParams.get('limit')) || 20000), selectionAudit(pool, 20)]); return reply(res,200,{ selection, variants, audit, brokerAutomation:'NONE', shadowOnly:true }); }
    if(url.pathname === '/api/strategies/latest-signal' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); const rows = (await pool.query('SELECT family,horizon_seconds,direction,signal_bucket,entry_price,entry_timestamp,strategy_hash,updated_at FROM frozen_latest_signal')).rows; return reply(res,200,{ signals: rows, shadowOnly:true, brokerAutomation:'NONE' }); }
    if(url.pathname === '/api/strategies/history' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); return reply(res,200,{ history: await frozenHistory(pool, Number(url.searchParams.get('limit')) || 200), shadowOnly:true }); }
    if(url.pathname === '/api/strategies/promotion' && req.method === 'GET') { if(req.headers['x-relay-admin'] !== admin) return reply(res,401,{error:'unauthorized'}); const state=(await pool.query('SELECT * FROM promotion_state WHERE id=1')).rows[0]??null; const audit=(await pool.query('SELECT * FROM promotion_audit ORDER BY created_at DESC LIMIT 50')).rows; return reply(res,200,{ state, audit, brokerAutomation:'NONE' }); }
    return reply(res,404,{error:'not_found'});
  } catch(error) { return reply(res,error.message==='payload_too_large'?413:400,{error:error.message}); }
});
function startFrozenLoop(pool){
  let busy=false;
