...
try {
  ...
  console.log("VISION_REAL", JSON.stringify({ status: vision?.provenance?.status, availability: vision?.availability, symbol: vision?.symbol, marketType: vision?.marketType, investmentValue: vision?.investmentValue, expirationSeconds: vision?.expirationSeconds, latencyMs: vision?.provenance?.latencyMs, notes: vision?.notes }));
