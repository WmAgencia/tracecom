...
export class IqMultiRuntime extends EventEmitter {
  ...
  async blitzStats(hours = 6) { if (!this.pool?.query) return { trades: 0 }; const bounded = Math.max(1, Math.min(72, Number(hours) || 6)); const row = (await this.pool.query("SELECT count(*) FILTER (WHERE result IN ('WIN','LOSS','DRAW'))::int AS settled, count(*) FILTER (WHERE result='WIN')::int AS wins, count(*) FILTER (WHERE result='LOSS')::int AS losses, count(*) FILTER (WHERE result='DRAW')::int AS draws, coalesce(sum(pnl) FILTER (WHERE result IN ('WIN','LOSS','DRAW')),0)::numeric AS pnl, coalesce(avg(payout) FILTER (WHERE result IN ('WIN','LOSS')),0)::numeric AS avg_payout FROM iq_lab_trades WHERE run_id='agentic-blitz-45s' AND entry_at >= now() - ($1 || ' hours')::interval", [String(bounded)]).catch(() => ({ rows: [] }))).rows?.[0] ?? null; if (!row) return { trades: 0 }; const decided = Number(row.wins) + Number(row.losses); const wr = decided > 0 ? Number((100 * Number(row.wins) / decided).toFixed(1)) : null; const breakeven = Number(row.avg_payout) > 0 ? Number((100 / (1 + Number(row.avg_payout) / 100)).toFixed(1)) : null; return { runId: "agentic-blitz-45s", expirationSeconds: BLITZ_EXPIRATION_SECONDS, settled: Number(row.settled), wins: Number(row.wins), losses: Number(row.losses), draws: Number(row.draws), winRate: wr, breakeven, edge: wr != null && breakeven != null ? Number((wr - breakeven).toFixed(1)) : null, pnl: Number(row.pnl), entriesPerHour: Number((Number(row.settled) / bounded).toFixed(1)) }; }
