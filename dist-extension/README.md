...
Version 0.4.6 adds bounded visual chart-header/quote resolution, nested passive IQ instrument metadata projection, and a dual confirmation gate (visible asset plus active-ID registry) before a shadow signal. **TRACE_1M**: analysis and expiry are fixed to one minute, the
...
`dist/tracecon-extension-v0.4.6.zip` and the unpacked `dist-extension` folder.
