...
exportButton.addEventListener("click", async () => { const response = await message("tc.experiment.status"); const blob = new Blob([JSON.stringify({ report: "TRACE_1M_A80_FINAL_REPORT", generatedAt: Date.now(), experiment: response?.data || null }, null, 2)], { type: "application/json" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = "trace1m-experiment-report.json"; link.click(); URL.revokeObjectURL(link.href); });
refresh();
