    ...
    if (path.startsWith("/api/iq/") && (req.method === "GET" || req.method === "POST" || req.method === "PUT")) {
      ...
      if (req.method === "GET") {
        ...
        const query = path === "/api/iq/executions"
          ...
          : path === "/api/iq/broker-audit" ? `?live=${q.get("live") === "0" ? "0" : "1"}${q.get("probe") ? `&probe=${encodeURIComponent(q.get("probe") as string)}` : ""}${q.get("orderProbe") === "1" ? "&orderProbe=1" : ""}`
