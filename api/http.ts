...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (path.startsWith("/api/iq/") && (req.method === "GET" || req.method === "POST" || req.method === "PUT")) {
      const getPaths = ["/api/iq/status", "/api/iq/executions", "/api/iq/office", "/api/iq/markets", "/api/iq/events", "/api/iq/asset-map", "/api/iq/diagnostic/options", "/api/iq/signals", "/api/iq/intelligence", "/api/iq/research/scoreboard", "/api/iq/manager", "/api/iq/audit", "/api/iq/apprentice", "/api/iq/apprentice/trades", "/api/iq/stress/report"];
      ...
      const putPaths = ["/api/iq/market", "/api/iq/manager/config", "/api/iq/apprentice/config"];
      ...
      const payload: Record<string, unknown> = path === "/api/iq/connect" ? { email: String(operationalInput.email ?? "").trim(), password: String(operationalInput.password ?? "") }
        ...
        : path === "/api/iq/manager/config" ? { mode: operationalInput.mode === undefined ? undefined : String(operationalInput.mode).slice(0, 32), autoSwitchEnabled: operationalInput.autoSwitchEnabled === undefined ? undefined : operationalInput.autoSwitchEnabled === true, reviewEverySettlements: operationalInput.reviewEverySettlements === undefined ? undefined : Number(operationalInput.reviewEverySettlements), minTotalSamples: operationalInput.minTotalSamples === undefined ? undefined : Number(operationalInput.minTotalSamples), minRecentSamples: operationalInput.minRecentSamples === undefined ? undefined : Number(operationalInput.minRecentSamples), minPerformanceDelta: operationalInput.minPerformanceDelta === undefined ? undefined : Number(operationalInput.minPerformanceDelta), maxDrawdown: operationalInput.maxDrawdown === undefined ? undefined : Number(operationalInput.maxDrawdown), minPayoutQuality: operationalInput.minPayoutQuality === undefined ? undefined : Number(operationalInput.minPayoutQuality), cooldownSettlements: operationalInput.cooldownSettlements === undefined ? undefined : Number(operationalInput.cooldownSettlements), minTimeBetweenSwitchesMs: operationalInput.minTimeBetweenSwitchesMs === undefined ? undefined : Number(operationalInput.minTimeBetweenSwitchesMs) }
      ...
      const result = await relayAdminJson(path === "/api/iq/market" || path === "/api/iq/manager/config" || path === "/api/iq/apprentice/config" ? "PUT" : "POST", path, payload, 20_000);
