...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (path.startsWith("/api/iq/") && (req.method === "GET" || req.method === "POST" || req.method === "PUT")) {
      ...
      const putPaths = ["/api/iq/market", "/api/iq/supervisor/config", "/api/iq/apprentice/config"];
      ...
      const result = await relayAdminJson(path === "/api/iq/market" || path === "/api/iq/supervisor/config" || path === "/api/iq/apprentice/config" ? "PUT" : "POST", path, payload, 20_000);
