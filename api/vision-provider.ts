...
type VisionInput = {
  ...
  frameId?: string | null;
  task?: "CHART_OBSERVATION" | "PRICE_LABEL_ONLY";
};
...
export class NexxusVisionProvider {
  ...
  async observe(input: VisionInput): Promise<Record<string, unknown>> {
    ...
    try {
      const text = input.task === "PRICE_LABEL_ONLY" ? `RETURN JSON ONLY. No markdown. Inspect only the white current-price label near the right price axis. Return {"price":number|null,"priceConfidence":number|null,"labelVisible":boolean,"bbox":{"x":number,"y":number,"width":number,"height":number}|null,"evidence":[]}. Preserve every decimal digit. Do not infer price from candle geometry or unrelated text. The input is a sanitized price-axis crop.` : `RETURN JSON ONLY. No markdown fences, no explanation. Return this shape: {"status":"AVAILABLE|PARTIAL|UNAVAILABLE","asset":{"value":string|null,"confidence":number|null},"symbol":string|null,"marketType":string|null,"timeframe":string|null,"timeframeSeconds":number|null,"visibleWindowSeconds":number|null,"price":number|null,"priceConfidence":number|null,"timestamp":number|null,"trend":"BULLISH|BEARISH|SIDEWAYS|UNCLEAR|null","structure":string|null,"momentum":"UP|DOWN|NEUTRAL|UNCLEAR|null","volatility":"LOW|NORMAL|HIGH|UNCLEAR|null","supportResistance":[],"breakoutState":"UP|DOWN|NONE|UNCLEAR|null","visibleCandleCount":number|null,"visualQuality":{"candlesReadable":boolean|null,"assetReadable":boolean|null,"priceReadable":boolean|null},"manualPosition":{"hasOpenPosition":boolean,"state":"NO_POSITION|POSSIBLE_POSITION|POSITION_CONFIRMED|WAITING_SETTLEMENT|SETTLED","direction":"BUY|SELL|UNKNOWN","confidence":number|null,"evidence":[]},"evidence":[]}. Only set manualPosition from the IQ Option position panel / open-position card (e.g. "Opções (n)", active operation card, ACIMA/ABAIXO marker of an OPEN operation, stake or expiration of the open operation). NEVER derive position direction from candle color, price movement, trend, regime, momentum or your own decision. If a position is visible but its direction is not readable, use state POSITION_CONFIRMED with direction UNKNOWN and hasOpenPosition true. Evidence strings must name the panel/card cues. Use null/UNCLEAR when unreadable. The image is a sanitized chart crop. Sanitized context: ${JSON.stringify(input.context ?? {}).slice(0, 4000)}`;
      ...
      if (manualState === "NO_POSITION" && manualRaw.hasOpenPosition === true) { /* position seen but state unreadable: keep state, let caller refine */ }
      const panelCue = Array.isArray(manualRaw.evidence) && manualRaw.evidence.some((item) => typeof item === "string" && /panel|posi|card|open|opera|expir|opç|opc|acima|abaixo|stake|invest/i.test(item));
      const manualDirection = panelCue && (manualRaw.direction === "BUY" || manualRaw.direction === "SELL") ? manualRaw.direction : "UNKNOWN";
      const observation = { symbol, marketType, timeframe, timeframeSeconds, visibleWindowSeconds: numberOrNull(value.visibleWindowSeconds), price, priceConfidence: numberOrNull(value.priceConfidence) ?? numberOrNull(value.confidence), priceSource: price === null ? "UNAVAILABLE" : "VISION_PRICE_LABEL", labelVisible: boolOrNull(value.labelVisible), bbox: value.bbox && typeof value.bbox === "object" ? value.bbox : null, timestamp, trend, structure: textOrNull(value.structure), momentum, volatility, supportResistance: Array.isArray(value.supportResistance) ? value.supportResistance.slice(0, 8) : [], breakoutState, visibleCandleCount: numberOrNull(value.visibleCandleCount), visualQuality: { candlesReadable: boolOrNull(visualQuality.candlesReadable), assetReadable: boolOrNull(visualQuality.assetReadable), priceReadable: boolOrNull(visualQuality.priceReadable) }, manualPosition: { hasOpenPosition: manualRaw.hasOpenPosition === true || panelCue, state: manualState, direction: manualDirection, confidence: numberOrNull(manualRaw.confidence), evidence: Array.isArray(manualRaw.evidence) ? manualRaw.evidence.filter((item): item is string => typeof item === "string").slice(0, 6) : [] }, evidence, availability, imageProvided: true, imageBytes: image!.byteLength, imageHash: image!.hash, parseMode: parsed.mode, sources: ["nexxus-vision", "sanitized-crop-base64"], notes: parsed.mode === "FAILED" ? ["VISION_INVALID_JSON"] : availability === "PARTIAL" ? [parsed.mode === "REPAIRED" ? "VISION_PARSE_REPAIRED" : "VISION_PARSE_DIRECT"] : ["IMAGE_PROVIDED_FIELDS_UNREADABLE"] };
