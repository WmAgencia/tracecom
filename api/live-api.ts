...
const browserEventTypes = new Set(["SESSION_STARTED", "VISION_MARKET_SAMPLE", "MARKET_OBSERVATION", "MARKET_CONTEXT_LOCKED", "MARKET_CONTEXT_INVALID", "DECISION", "COUNTDOWN", "SHADOW_UPDATE", "SETTLEMENT", "PIPELINE_ERROR", "HEARTBEAT", "SESSION_ENDED", "OPERATIONAL_SIGNAL_CREATED", "OPERATIONAL_SIGNAL_LOCKED", "OPERATIONAL_DECISION_SUPPRESSED_ACTIVE_OPERATION", "OPERATIONAL_DECISION_SUPPRESSED_CONTEXT_INVALID", "ENTRY_COUNTDOWN_STARTED", "ENTRY_CONFIRMATION_WAITING", "POSITION_CONFIRMED", "MANUAL_ENTRY_PRICE_LOCKED", "ENTRY_NOT_CONFIRMED", "SIGNAL_INVALIDATED_BEFORE_ENTRY", "SETTLEMENT_PRICE_LOCKED", "TRADE_SETTLED", "COOLDOWN_STARTED", "OPERATIONAL_CHANNEL_READY"]);
...
export async function handleLiveApi(req: IncomingMessage, res: ServerResponse, path: string, body: unknown, query: URLSearchParams): Promise<boolean> {
  ...
  if (path === "/api/live/browser/ingest" && req.method === "POST") {
    ...
    if (!input || typeof input.sessionId !== "string" || !/^vision_[a-zA-Z0-9_-]{8,100}$/.test(input.sessionId) || typeof input.type !== "string" || !browserEventTypes.has(input.type) || containsSensitive(input)) { const reason = !input ? "payload_missing" : typeof input.sessionId !== "string" ? "session_id_missing" : !/^vision_[a-zA-Z0-9_-]{8,100}$/.test(input.sessionId) ? "session_id_invalid" : typeof input.type !== "string" ? "event_type_missing" : !browserEventTypes.has(input.type) ? "event_type_not_allowed" : "sensitive_field"; const field = reason.includes("session") ? "sessionId" : reason.includes("event_type") ? "type" : reason === "sensitive_field" ? "payload" : null; send(res, 400, { error: "invalid_browser_telemetry", code: "BROWSER_INGEST_REJECTED", reason, field, eventType: typeof input?.type === "string" ? input.type : null, sessionId: typeof input?.sessionId === "string" ? input.sessionId : null }); return true; }
