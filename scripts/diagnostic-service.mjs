/* Local-only DEV service (OpenCode Go). API credentials stay in this process environment. */
import http from "node:http";

const port = Number(process.env.TRACECON_DIAGNOSTIC_PORT || 8789);
const baseUrl = String(process.env.OPENCODE_GO_BASE_URL || "https://opencode.ai/zen/go/v1").replace(/\/$/, "");
const apiKey = process.env.OPENCODE_GO_API_KEY;
const configuredModel = String(process.env.AI_MODEL || "deepseek-v4.1-flash").trim();
...
const sanitize = (value, depth = 0) => {
  ...
  if (value === null || ["string", "number", "boolean"].includes(typeof value)) return typeof value === "string" ? redactSecrets(value.slice(0, 240)) : value;
...
async function resolveModel() {
  ...
  const fallback = { provider: "OPENCODE_GO", displayName: configuredModel, modelId: configuredModel };
  const response = await boundedFetch(`${baseUrl}/models`, { headers: { Authorization: `Bearer ${apiKey}` } });
  if (!response.ok) return fallback;
  const catalog = await response.json();
  const models = Array.isArray(catalog?.data) ? catalog.data : Array.isArray(catalog) ? catalog : [];
  const model = models.find((item) => item?.id === configuredModel)
    || models.find((item) => /deepseek/i.test(`${item?.id || ""} ${item?.display_name || item?.name || ""}`));
  if (!model?.id) return fallback;
  return { provider: "OPENCODE_GO", displayName: model.display_name || model.name || model.id, modelId: model.id };
}
...
async function askModel(profile, snapshot, chartImage = null) {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    delete decoded.chartImage; const snapshot = sanitize(decoded); const profile = req.url === "/diagnose" ? "FABLE_DIAGNOSTIC" : "FABLE_TRADER"; const answer = await askModel(profile, snapshot, chartImage);
