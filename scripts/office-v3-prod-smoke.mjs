...
async function main() {
  ...
  try {
    ...
    assert("V2 ORIGINAL controla a execucao (Strategy Core V2 + infra atual); V4 em shadow", Boolean(boot.rsiV2Live) && boot.rsiV2Live.routing === "RSI_V2_ONLY" && strategySet.length >= 1 && strategySet.every((id) => id === "RSI_REVERSAL_STRICT_V2" || id === "RSI_EXTREME_PULLBACK_V2") && boot.desksWithTag >= 1 && boot.rsiV4?.routing === "RSI_V4_ONLY" && boot.rsiV4?.enabled === true, "final-prod-boot.png", { v2Routing: boot.rsiV2Live?.routing, deskStrategies: strategySet, desksWithTag: boot.desksWithTag, v4: boot.rsiV4?.routing });
