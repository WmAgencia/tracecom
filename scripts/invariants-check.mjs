...
const v3Manifest = JSON.parse(read("estrategias/strategy-versions/PULLBACK_4060_300_AGENTIC_V3.json"));
ok("ONE_OPERATIONAL_STRATEGY", v2Manifest.strategyVersion === "PULLBACK_4060_300_AGENTIC_V2" && v2Manifest.status === "ACTIVE" && v2Manifest.executable === true && v3Manifest.strategyVersion === "PULLBACK_4060_300_AGENTIC_V3" && v3Manifest.status === "PENDING_IMPLEMENTATION" && v3Manifest.executable === false && fs.readdirSync(new URL("../estrategias/strategy-versions", import.meta.url)).filter((f) => f.endsWith(".json") && !f.endsWith(".freeze.json")).length === 2);
ok("V3_STARTS_DISABLED", v3Manifest.executable === false && v3Manifest.status !== "ACTIVE" && /V3_EXECUTE/.test(read("relay/v3/runtime.mjs")) && /OBSERVE_ONLY/.test(read("relay/v3/runtime.mjs")));
ok("ONE_OPERATIONAL_BROKER_PATH", (runtime.match(/client\.placeOrder\(/g) ?? []).length === 1 && operationalAllowlist().filter((s) => s.startsWith("intelligence:")).length === 1 && operationalAllowlist().length === 1 && !operationalAllowlist().some((s) => s.startsWith("lab:") || s.startsWith("pathtest:") || s === "ui:smoke") && OPERATIONAL_EXECUTION_POLICY_NAME === "OPERATIONAL_V2_SINGLE_PATH");
