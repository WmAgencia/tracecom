...
function features(candles, i) {
  ...
  return { closes, n, c, rsi: rsiAt(closes, n - 1, 14), rsi3: rsiAt(closes, n - 1, 3), rsiPrev: rsiAt(closes, n - 2, 14), mom30: ret(6), mom60: ret(12), mom120: ret(24), slope: ema9 !== null && ema21 ? (ema9 - ema21) / ema21 : null, ema9, ema21, macdHist, macdPrev, stochK: stochK(c, i), stochKPrev: stochK(c, i - 1), cci: cci(c, i), cciPrev: cci(c, i - 1), willr: willr(c, i), willrPrev: willr(c, i - 1), srs, srsPrev, vol, bb: bbOf(closes), fib: fibLevels(c, i) };
}
function evaluate(f) {
  const out = [];
  ...
  { let buy = false, sell = false; if (f.bb) { const last = f.closes[f.n - 1]; buy = last <= f.bb.lower && f.rsi !== null && f.rsi < 30; sell = last >= f.bb.upper && f.rsi !== null && f.rsi > 70; } out.push({ st: "bollinger-rsi-v1", dir: buy ? "BUY" : sell ? "SELL" : null }); }
...
const fibOk = (f, dir) => { if (!f.fib) return false; return dir === "BUY" ? f.fib.upSwing && f.fib.inZone : !f.fib.upSwing && f.fib.inZone; };
...
(async () => {
    ...
    for (let i = MIN_CANDLES; i < candles.length; i++) {
      ...
      for (const h of [45_000, 60_000]) {
        ...
        for (const sig of sigs) {
          ...
          if (fibOk(f, sig.dir)) trades.push({ st: sig.st, variant: "fib", dir: sig.dir, t: ref.t, h, result: outcome(sig.dir, ref.v, settle.v) });
        }
      }
    }
  }
  ...
  const key = (r) => `T+${r.h}s`;
  const bySt = new Map();
  for (const r of summary) { if (!bySt.has(r.st)) bySt.set(r.st, []); bySt.get(r.st).push(r); }
  console.log("strategy | base T+45 (n,WR) | fib T+45 (n,WR) | delta | base T+60 | fib T+60 | delta");
  const rowsOut = [];
  for (const [st, list] of bySt) {
    const g = (v, h) => list.find((r) => r.variant === v && r.h === h);
    const b45 = g("base", 45), f45 = g("fib", 45), b60 = g("base", 60), f60 = g("fib", 60);
    const dd = (a, b) => (a && b && a.wr !== null && b.wr !== null ? +(b.wr - a.wr).toFixed(1) : null);
    const line = { st, base45: b45 && [b45.n, b45.wr], fib45: f45 && [f45.n, f45.wr, f45.ciLow], delta45: dd(b45, f45), base60: b60 && [b60.n, b60.wr], fib60: f60 && [f60.n, f60.wr, f60.ciLow], delta60: dd(b60, f60) };
    rowsOut.push(line);
    console.log(`${st} | ${b45 ? b45.n + "," + b45.wr : "-"} | ${f45 ? f45.n + "," + f45.wr : "-"} | ${line.delta45} | ${b60 ? b60.n + "," + b60.wr : "-"} | ${f60 ? f60.n + "," + f60.wr : "-"} | ${line.delta60}`);
  }
  fs.writeFileSync(path.join(OUT_DIR, "fib-all-summary.json"), JSON.stringify(rowsOut, null, 1), "utf8");
  await c.end();
