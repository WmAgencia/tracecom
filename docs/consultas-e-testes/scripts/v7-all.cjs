...
function decide(strategy, f) {
  if (!f) return "WAIT";
  const atrDir = f.close < f.sma20 ? "BUY" : f.close > f.sma20 ? "SELL" : "WAIT";   // componente ATR sempre-disponível (contra-desvio)
  const v1Dir = f.s > 0 ? "BUY" : f.s < 0 ? "SELL" : "WAIT";                        // componente V1 sempre-disponível (contra-exaustão)
  switch (strategy) {
    case "v7relaxed": { const a = atrfib(f, 1), b = v1fib(f); if (a === "WAIT") return b; if (b === "WAIT") return a; return a === b ? a : "WAIT"; }
    case "v7allagree": return atrDir !== "WAIT" && v1Dir === atrDir ? atrDir : "WAIT";
    case "v7allatr": return atrDir === "WAIT" ? "WAIT" : atrDir;
    case "v7allv1": return v1Dir === "WAIT" ? "WAIT" : v1Dir;
    case "v7allfib": { const d = atrDir === "WAIT" ? v1Dir : atrDir; return d; }
    default: return "WAIT";
  }
}
...
const NAMES = { v7relaxed: "V7-relaxado (ATR+Fib dev>=1xATR OU V1+Fib)", v7allagree: "V7-all-AGREE (todos: ATR e V1 concordam; conflito=WAIT)", v7allatr: "V7-all-ATR (todos: contra-SMA20)", v7allv1: "V7-all-V1 (todos: contra-RSI)", v7allfib: "V7-all (todos: ATR com fib de contexto)" };
const STRATS = Object.keys(NAMES);
(async () => {
  await c.connect();
  ...
  require("fs").writeFileSync("C:/tracecom-forward4/v7-all.json", JSON.stringify({ evaluated, skipped, res }, null, 1));
  await c.end();
