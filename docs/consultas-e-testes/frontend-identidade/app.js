...
const state = { stream: null, crop: null, selecting: false, start: null, frames: [], observations: [], priceObservations: [], priceOutlierCandidates: [], priceTimer: null, marketObserver: null, priceAnalyzing: false, timer: null, countdownTimer: null, analyzing: false, history: loadHistory(), lastAnalysis: null, training: null, lastContext: null, marketContext: null, contextSegmentId: null, manualPositionConfirmations: 0, manualPosition: { state: "NO_POSITION", direction: "UNKNOWN", confidence: 0, evidence: [] }, metrics: { analysisStarted: 0, analysisCompleted: 0, analysisStale: 0, analysisSkippedBusy: 0, observerCycles: 0, observerSkipped: 0, observerOverlap: 0, priceObservationsValid: 0, priceObservationsRejected: 0 }, stage: "IDLE", session: 0, liveSessionId: null, liveSequence: 0, failures: 0, nextRetryAt: 0, circuitOpen: false, entryUntil: 0, reanalysisAt: 0, decisionId: null, decisionSource: null, deepRequested: false, deepExecuted: false, decisionTimestamp: 0, expirationSeconds: EXPIRATION_SECONDS, lastSubmittedHash: null, lastSubmittedAt: 0, lastCandleId: null, requestCount: 0, hasSuccessfulAnalysis: false };
function lockMarketContext(observation, frameId) { const raw = String(observation?.symbol || observation?.asset || "").toUpperCase().replace(/\s+/g, " ").trim(); const cleaned = /UNAVAILABLE|UNREADABLE|UNKNOWN|NO ?DATA|NOT ?VISIBLE/.test(raw) ? "" : raw; const pair = cleaned.match(/([A-Z]{3})\s*\/\s*([A-Z]{3})/) || cleaned.match(/([A-Z]{6})/); const pairCanonical = pair ? (pair[1].length === 6 ? `${pair[1].slice(0, 3)}/${pair[1].slice(3)}` : `${pair[1]}/${pair[2]}`) : null; const asset = pairCanonical ? pairCanonical : null; const marketType = String(observation?.marketType || "").toUpperCase() === "OTC" || /\bOTC\b/.test(cleaned) ? "OTC" : "UNKNOWN"; const timeframe = Number(observation?.timeframeSeconds); const visible = Number(observation?.visibleWindowSeconds); const expiration = EXPIRATION_SECONDS; const status = asset && marketType === "OTC" && timeframe === CANDLE_SECONDS && visible === 300 && expiration === 60 ? "VALID" : !asset ? "INSUFFICIENT_EVIDENCE" : marketType !== "OTC" ? "MARKET_TYPE_MISMATCH" : timeframe !== CANDLE_SECONDS ? "TIMEFRAME_MISMATCH" : visible !== 300 ? "VISIBLE_WINDOW_MISMATCH" : "EXPIRATION_MISMATCH"; const [baseCurrency, quoteCurrency] = asset ? asset.split("/") : [null, null]; const changed = state.marketContext?.assetCanonical !== asset || state.marketContext?.marketType !== marketType; if (changed) { state.contextSegmentId = `seg_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`; state.priceObservations = []; state.priceOutlierCandidates = []; state.observations = []; state.lastCandleId = null; state.deepContext = { version: 0, at: 0, observation: null, trend: null, momentum: null, regime: null, latencyMs: null }; state.entryUntil = 0; state.reanalysisAt = 0; }
...
function priceCrop() {
  const video = $("screenVideo"); if (!video.videoWidth) return null;
  return makeCrop(detectPriceLabelBox(video), 220);
}
// Dynamic price-label detector (no manual crop needed): scans the right-side band
// for the white price marker label and returns its box, so a vertically-moving
// price is always captured. Falls back to a wide static box when detection fails.
function detectPriceLabelBox(video) {
  const W = video.videoWidth, H = video.videoHeight;
  const bx = Math.round(.855 * W), bw = Math.max(1, Math.round(.095 * W));
  const by = Math.round(.04 * H), bh = Math.max(1, Math.round(.88 * H));
  const cw = Math.max(8, Math.round(bw * .5)), ch = Math.max(8, Math.round(bh * .5));
  const fallback = { x: .855, y: .40, width: .095, height: .30 };
  try {
    const canvas = document.createElement("canvas"); canvas.width = cw; canvas.height = ch;
    const ctx = canvas.getContext("2d", { willReadFrequently: true }); if (!ctx) return fallback;
    ctx.drawImage(video, bx, by, bw, bh, 0, 0, cw, ch);
    const data = ctx.getImageData(0, 0, cw, ch).data;
    const rows = new Array(ch).fill(0);
    for (let y = 0; y < ch; y++) { let white = 0; for (let x = 0; x < cw; x++) { const i = (y * cw + x) * 4; if (data[i] > 200 && data[i + 1] > 200 && data[i + 2] > 195) white++; } rows[y] = white; }
    const thr = Math.max(3, Math.round(cw * .12));
    let best = null, a = -1;
    for (let y = 0; y <= ch; y++) {
      const isWhite = y < ch && rows[y] >= thr;
      if (isWhite && a === -1) a = y;
      if (!isWhite && a !== -1) { const len = y - a; if (!best || len > best.len) best = { a, b: y - 1, len }; a = -1; }
    }
    if (!best || best.len < 2) return fallback;
    const pad = 2, y1 = Math.max(0, best.a - pad), y2 = Math.min(ch - 1, best.b + pad);
    return { x: .855, y: .04 + (y1 / ch) * .88, width: .095, height: Math.max(.06, ((y2 - y1 + 1) / ch) * .88) };
...
class BrowserMarketObserver { constructor(capture, intervalMs = 1000) { this.capture = capture; this.intervalMs = intervalMs; this.timer = null; this.running = false; this.stopped = true; } start() { if (!this.stopped) return; this.stopped = false; void this.cycle(); } stop() { this.stopped = true; if (this.timer) clearTimeout(this.timer); this.timer = null; } async cycle() { if (this.stopped) return; if (this.running) { state.metrics.observerSkipped += 1; state.metrics.observerOverlap += 1; this.timer = setTimeout(() => void this.cycle(), this.intervalMs); return; } this.running = true; state.metrics.observerCycles += 1; const started = Date.now(); try { await this.capture(); } finally { this.running = false; this.timer = setTimeout(() => void this.cycle(), Math.max(0, this.intervalMs - (Date.now() - started))); } } }
function startPriceTracking() { if (state.marketObserver) state.marketObserver.stop(); state.marketObserver = new BrowserMarketObserver(trackPrice, 1000); state.marketObserver.start(); }
...
// Calibrated auto-region: covers the instrument name/OTC header (top-left),
// the full chart and the right-side price marker area. No manual crop required.
function detectChartRegion(video, surface = "browser") {
  const ratio = video.videoWidth / Math.max(1, video.videoHeight);
  const crop = { x: .02, y: .06, width: .945, height: .835 };
  const geometryOk = ratio >= 1.2 && ratio <= 2.4;
  const base = geometryOk ? .9 : .6;
  const confidence = surface === "browser" ? base : Math.min(base, .85);
  return { crop, confidence, source: `LAYOUT_GEOMETRY_V2_${String(surface).toUpperCase()}` };
}
...
async function shareScreen() {
  ...
  try {
    ...
    try { localStorage.removeItem(OPERATIONAL_SESSION_KEY); localStorage.removeItem(OPERATIONAL_CURSOR_KEY); localStorage.removeItem("tracecom:training-session"); localStorage.removeItem(TRAINING_CONFIG_KEY); localStorage.removeItem("tracecom:vision-history"); } catch { /* best-effort reset */ }
    state.operationalSessionId = null; state.operationalCursor = 0; state.channel = null; state.training = null;
...
function stopScreen() {
  ...
  if (state.timer) clearTimeout(state.timer); state.timer = null; if (state.marketObserver) state.marketObserver.stop(); state.marketObserver = null; if (state.priceTimer) clearInterval(state.priceTimer); state.priceTimer = null; if (state.deepTimer) clearInterval(state.deepTimer); state.deepTimer = null; state.deepRunning = false;
...
function headerCrop() { return makeCrop({ x: .015, y: .03, width: .38, height: .16 }, 1000); }
// Composite context strip: instrument header (top-left) + timeframe label (bottom-left)
// stacked vertically in a single high-resolution image, so the analyzer can read
// the symbol, the OTC marker AND the candle interval without confusing them.
function contextCrop() {
  const video = $("screenVideo"); if (!video.videoWidth || !video.videoHeight) return null;
  try {
    const parts = [{ x: .015, y: .03, width: .38, height: .16 }, { x: .02, y: .655, width: .10, height: .15 }];
    const widths = parts.map((p) => Math.round(p.width * video.videoWidth));
    const W = Math.min(1120, Math.max(...widths));
    const heights = parts.map((p) => Math.max(1, Math.round((p.height * video.videoHeight) * (W / Math.max(1, p.width * video.videoWidth)))));
    const H = heights.reduce((s, h) => s + h, 0) + 6;
    const canvas = document.createElement("canvas"); canvas.width = W; canvas.height = H;
    const ctx = canvas.getContext("2d", { alpha: false }); if (!ctx) return headerCrop();
    let y = 0;
    for (let i = 0; i < parts.length; i++) {
      const p = parts[i], sw = Math.round(p.width * video.videoWidth), sh = Math.round(p.height * video.videoHeight);
      ctx.drawImage(video, Math.round(p.x * video.videoWidth), Math.round(p.y * video.videoHeight), sw, sh, 0, y, W, heights[i]);
      y += heights[i] + 6;
    }
    return { dataUrl: canvas.toDataURL("image/jpeg", .85), width: W, height: H };
  } catch { return headerCrop(); }
}
...
async function observe() {
  ...
  try {
    ...
    if (fast.fibProfiles) { const fp = fast.fibProfiles; text("profilesLine", `V1 ${fp.v1} · V2 ${fp.v2} · V3 ${fp.v3} · V6 ${fp.v6} · concordantes ${fp.concordantCount} ${fp.concordantDirection}`); }
...
async function deepAnalyze() {
  ...
  try {
    ...
    const result = await api("/api/fable/trade", { method: "POST", body: JSON.stringify({ snapshot, chartImages: temporal, contextImage: contextCrop()?.dataUrl || null }) });
    ...
    void liveEmit("DECISION", { decisionId: state.decisionId, sessionId: state.liveSessionId, traceId: state.currentTraceId, marketEventId: state.lastCandleId, candleId: state.lastCandleId, frameId, decisionSource: "FAST_THEN_DEEP", deepRequested: true, deepExecuted: true, direction: analysis.decision || "WAIT", confidence: analysis.confidence ?? null, probabilitySource: analysis.probabilitySource || "DEEP_ANALYSIS", agentRunIds: analysis.agentRunIds || [] });
...
// ===== Identidade visual das estratégias (camada APENAS de apresentação) =====
// IDs internos preservados como fonte de verdade; displayName é rótulo visual.
// Perfis são METADADOS de categoria — NÃO afirmam WR futuro e NÃO mudam com resultados.
const STRATEGY_IDENTITY = [
  { internalId: "reversion-v7-and", internalLabel: "V7-AND", displayName: "Fib Dual Exhaustion", profile: "CONVICÇÃO MÁXIMA", description: "Concordância entre overshoot de ATR (>=3xATR da SMA20) e exaustão de RSI, ambos na zona dourada." },
  { internalId: "reversion-v6-fib", internalLabel: "V6+Fib", displayName: "Fib Deep Exhaustion", profile: "CONSERVADORA", description: "Exaustão profunda de RSI (banda 0,63-0,857), excluindo o extremo terminal, na zona dourada." },
  { internalId: "reversion-v1-fib", internalLabel: "V1+Fib", displayName: "Fib RSI Reversal", profile: "BALANCEADA", description: "Exaustão de RSI(14) em região estrutural de Fibonacci com volatilidade baixa." },
  { internalId: "reversion-v3-fib", internalLabel: "V3+Fib", displayName: "Fib Trend Reversal", profile: "AGRESSIVA", description: "Reversão de RSI(14) alinhada à tendência de 120s, ancorada em Fibonacci." },
  { internalId: "reversion-v7-relaxed", internalLabel: "V7-Relaxado", displayName: "Fib Adaptive Reversal", profile: "ALTA FREQUÊNCIA", description: "União adaptativa: overshoot de ATR (>=1xATR) OU exaustão de RSI, em Fibonacci; conflito vira WAIT." },
];
const STRATEGY_FILTERS = ["Todas", "CONVICÇÃO MÁXIMA", "CONSERVADORA", "BALANCEADA", "AGRESSIVA", "ALTA FREQUÊNCIA"];
let strategyFilterActive = "Todas";
function renderStrategyIdentityPanel() {
  const filtersBox = $("strategyFilters"), cardsBox = $("strategyCards");
  if (!filtersBox || !cardsBox) return;
  filtersBox.innerHTML = STRATEGY_FILTERS.map((f) => `<button type="button" class="idFilter" data-filter="${f}" style="margin:2px;padding:2px 9px;border-radius:12px;border:1px solid rgba(255,255,255,.25);background:${f === strategyFilterActive ? "rgba(255,140,0,.35)" : "transparent"};color:inherit;font-size:11px;cursor:pointer">${f}</button>`).join("");
  filtersBox.querySelectorAll(".idFilter").forEach((btn) => btn.addEventListener("click", () => { strategyFilterActive = btn.dataset.filter; renderStrategyIdentityPanel(); }));
  const list = STRATEGY_IDENTITY.filter((s) => strategyFilterActive === "Todas" || s.profile === strategyFilterActive);
  cardsBox.innerHTML = list.map((s) => `<div class="idCard" style="border:1px solid rgba(255,255,255,.18);border-radius:8px;padding:6px 10px;margin:4px 0;background:rgba(255,255,255,.03)"><div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline"><b>${s.displayName}</b><span class="badge" style="font-size:10px">${s.profile}</span></div><div class="fine" style="opacity:.85">${s.description}</div><div class="fine" style="opacity:.55;font-size:10px" title="Identificador interno preservado para rastreabilidade">ID interno: ${s.internalLabel} (${s.internalId})</div></div>`).join("");
}
// ===== fim da camada de identidade =====
